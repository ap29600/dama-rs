...
pub fn get_config_lines(config: &str) -> impl Iterator<Item = &str> {
    config
        .split('\n')
        .into_iter()
...
pub fn execute_shell_command(command: &str) -> bool {
...
pub fn read_stdout_from_command(command: &str) -> String {
    match Command::new("sh").arg("-c").arg(command).output() {
        ...
        Ok(Output {
            ...
            Err(e) => {
                println!("error while making a string: {}", e);
                "".to_string()
            }
            Ok(string) => string,
        },
...
pub fn read_value_from_command<T>(command: &str) -> Option<T>
where
    T: std::str::FromStr,
    <T as FromStr>::Err: Debug,
{
    match &*read_stdout_from_command(command) {
        ...
        output => output
            .split('\n')
            .next()
            .and_then(|line| match line.parse() {
                Ok(val) => Some(val),
                Err(e) => {
                    eprint!("{:?}", e);
                    None
